...
# The HuggingFaceEmbeddings class has moved to a new package.
# This import is updated to resolve the deprecation warning.
from langchain_huggingface import HuggingFaceEmbeddings
# We are adding a new retriever to improve search
...
def create_rag_chain(vectorstore, llm_model):
    """
    Creates the complete RAG chain for question answering, using a MultiQueryRetriever.
    """
    if vectorstore is None:
        return None

    # The prompt for the MultiQueryRetriever.
    # It generates multiple perspectives on the user's question.
    QUERY_PROMPT = ChatPromptTemplate.from_messages(
        [
            ("human", "You are an AI language model assistant. Your task is to generate five different versions of the given user question to retrieve relevant documents from a vector database. By generating multiple perspectives on the user's query, your goal is to help the user overcome some of the limitations of the distance-based similarity search. Provide these alternative questions separated by newlines."),
            ("human", "{question}"),
        ]
    )

    # We are using a MultiQueryRetriever to improve the retrieval step
    retriever = MultiQueryRetriever.from_llm(
        retriever=vectorstore.as_retriever(), llm=llm_model, prompt=QUERY_PROMPT
    )
